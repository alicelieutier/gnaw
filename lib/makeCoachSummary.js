...
async function makeCoachSummary(timestamp, getTasks, getFloorCover, lookupCoachHandle) {
  const tasks = await getTasks(
    "Upcoming",
    `IS_SAME({End time}, DATETIME_PARSE(${timestamp}, 'x'), 'day')`
  );
  const cover = await getFloorCover(timestamp);
  return [
    "Today's tasks:",
    createTodaysTasksMessage(tasks, lookupCoachHandle),
    createFloorCoverMessage(cover, lookupCoachHandle)
  ]
...
function createFloorCoverMessage(cover, handleLookup) {
  const morningCoaches = formatCoachesList(cover.morning, handleLookup);
  const afternoonCoaches = formatCoachesList(cover.afternoon, handleLookup);
  return `Floor cover: Morning ${morningCoaches}. Afternoon ${afternoonCoaches}.`;
}
